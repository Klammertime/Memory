...
// The following function will be called whenever the
...
function servePage(request, response){
  ...
  // If the user does not enter a file name,
  // we server the page home.html
  if(filename === './'){
    filename = HOME;
  }
  // Get the extension of the requested resource so
  ...
  fs.readFile(filename, function(err, content){
    if(err){ // If there is an error, set the status code
      ...
      response.write(err.message);
       // Include the error message body
